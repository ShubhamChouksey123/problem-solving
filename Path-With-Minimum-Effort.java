...
31                int x = nodeX + direction[0]; 
32                int y = nodeY + direction[1];
33
34                if(x < 0 || x >= n || y < 0 || y >= m) continue;
35
36                int newEffort = Math.max(effort , Math.abs(heights[x][y] - heights[nodeX][nodeY] ));
37
38                if(newEffort < minEfforts[x][y]){
39                    minEfforts[x][y] = newEffort; 
40                    pq.add(new int[]{x, y, newEffort});
41                }
42            }
43        }
44        return minEfforts[n-1][m-1];
45    }
46}